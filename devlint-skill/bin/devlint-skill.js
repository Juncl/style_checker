 ...
 *   devlint-skill collect-design --code <传送码> | --url <设计稿URL> --path <工程目录>
 *   devlint-skill ui-style-check --design-json <path> --dev-json <path> [--platform hmPhone] [--design-image <path>] [--dev-image <path>]
 *   devlint-skill design-spec-check --source <html路径或URL> --spec <规范名>
 *
...
async function cmdCollectDesign(args) {
  const { code, url, path: filePath } = args
  if (!code && !url) fail('缺少参数: --code <传送码> 或 --url <设计稿URL> 至少传一个')
  if (!filePath) fail('缺少必填参数: --path <工程目录>')
  const result = await collectDesign(code, url, filePath)
  output(result)
}
...
async function cmdDesignSpecCheck(args) {
  const { source, spec: specName } = args
  if (!source) fail('缺少必填参数: --source')
  if (!specName) fail('缺少必填参数: --spec')
  const report = await uxCheck(source, specName)
  output(report)
}
...
命令:
  ...
  design-spec-check    检查 HTML/URL 是否符合设计规范
