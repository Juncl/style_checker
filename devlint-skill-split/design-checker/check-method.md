...
- 🔴 规范文件 > 3 个时禁止直接逐字通读——先跑脚本粗筛拿候选清单，再分批精读（见下节「规范粗筛与分批」）。
...
- 🔴 分批检查（见下节）时本遍必须**全局执行**——同类元素一致性需要全局视野，不能跟着分批走，放在全部批次完成后统一收口。

**分区逐块（确保不漏区域）**：
按 顶部导航栏 → 标题区 → 搜索/筛选/Tab → 内容区 → 列表项 → 底部操作栏 → 弹窗/浮层（如有）的顺序，逐块过一遍三遍法。

## 规范粗筛与分批（规范文件 > 3 个时，🔴 必走）

**为什么**：规范文档全文涌入上下文会挤占注意力——后续 issues 的格式要求和流程纪律会被大量规范内容"挤出"焦点，导致字段失守、跳步骤。用行级候选清单替代全文通读、每批读完立即产出，是对抗注意力稀释的手段。
...
- 输出：每个规范文件的疑似规则行清单（文件 + 行号 + 原文行），按"数值/颜色精确召回 + 规范关键词兜底召回"两路正则提取，宁多勿漏，精读时剔除误召回
- `specQuote` 优先从粗筛清单 / 规范原文复制，减少转写错误

**分批精读 + 即时产出**：

1. 按文件分组分批，每批 3-5 个文件
2. 每批流程：Read 该批规范文件（重点核对候选行及其上下文）→ 对检查范围执行第 2 遍逐元素检查 → **立即产出该批 issues 草稿**（暂存上下文，不递交脚本）
3. 🔴 禁止全部批次读完再统一产出——每批读完必须立即产出该批草稿，防止规则细节被后续批次冲淡
4. 全部批次完成后执行第 3 遍全局交叉验证收口（补齐跨批次遗漏，如批 1 的按钮与批 3 的按钮规格不一致）
5. 汇总全部批次 issues 草稿 + 交叉验证补漏 → 一次性经 stdin 递交脚本（递交格式见「输出格式」节，不变）
...
**第 2 部分：问题数据**（🔴 不落 AI 手——经 stdin（heredoc）递交 build-report.mjs，存档与报告由脚本完成）
...
递交方式（heredoc，引号形式的 `<<'JSON'` 防止内容被 shell 展开）：
...
（存档与报告由脚本完成；`--out-dir` 仅当用户指定过报告保存目录时由 AI 整理成**绝对目录**后传入——
mac 如 `~/reports` 展开 `/Users/<name>/reports`；Windows 如 `%USERPROFILE%\reports` 或 `~\reports`
展开 `C:\Users\<name>\reports`；相对路径转绝对，目录无需预建；未指定时不传，报告默认落用户主目录下
（mac：`~/.octo-uxlint/design-check/`；Windows：`C:\Users\<name>\.octo-uxlint\design-check\`）、
7 天自动过期；stdout 返回报告路径、JSON 存档路径、工作目录与问题统计。）
...
6. 无问题时，issues 为空数组 `[]`，文字总结写"前端实现符合设计规范，未发现问题"（🔴 0 问题同样把数据递交脚本——md 报告必出，见 SKILL.md）。
