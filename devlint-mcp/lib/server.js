...
import { loadDesignJsonAsBlob } from './collectData/getPixData/loadDesignJson.js'
...
export function createMcpServer() {
  ...
  mcp.tool(
    ...
    [
      ...
      '',
      '【硬性规则：采集只能用本 MCP 自带的工具】',
      '触发 ui_style_check 后，开发侧和设计侧的数据采集只能用本 MCP 自带的 collect_arkui / collect_web / collect_design 工具，',
      '禁止调用其他 MCP 或 skill 来采集数据。如果用户已有本地 JSON/图片文件路径，可直接传入本工具，无需采集。',
      '',
      '【用户未提供任何数据时的引导】',
      '当用户只说"帮我做 UI 检查/一致性检查"但没提供任何数据时，不要直接调用本工具，',
      '先向用户列出需要提供的信息，引导用户逐项补充：',
      '',
      '  本工具需要两份数据，缺一不可：',
      '',
      '  ① 开发侧数据（提供其一即可）：',
      ...
      '  ② 设计侧数据（提供其一即可）：',
      '     · 本地文件路径（design.json，附截图 png/jpg）',
      '     · 传送码（如 "111"）或设计稿 URL → 调 collect_design 自动采集',
      '  ③ 平台类型（可选，默认 hmPhone）：hmPhone（鸿蒙手机）/ hmWatch（鸿蒙手表）/ web（Web 网页）',
      '',
      '  请把上述清单整理成易读的文字发给用户，等用户逐项提供、两侧数据都就绪后再调用本工具。',
      '  如果用户只提供了一侧数据，继续追问缺失的那一侧，不要只拿一侧数据就调用。',
      `  如需详细使用指引，可向用户提供使用指南链接：${config.MCP_GUIDE_URL}`,
      '',
      '【调用前：数据来源判断】',
  ...
  mcp.tool(
    ...
    [
      ...
      '',
      '【用户未提供页面地址时的引导】',
      '当用户只说"采集网页/采集前端页面"但没给任何页面地址时，不要直接调用本工具，',
      '先向用户说明需要提供以下信息：',
      '',
      '  · 待采集的页面地址（必填），二选一：',
      '     - 线上网页 URL（http:// 或 https:// 开头）',
      '     - 本地 HTML 文件路径（如 /path/to/page.html）',
      '  · 视口尺寸（可选，默认 1920×1080）',
      '  · 是否需要弹出浏览器窗口手动操作（如需登录，传 headless=false）',
      '',
      '  请把上述清单发给用户，等用户给出页面地址后再调用本工具。',
      '',
      '【参数说明】',
  ...
  mcp.tool(
    'collect_design',
    [
      '设计稿数据采集工具。通过传送码或设计稿 URL，采集设计稿的节点树数据 + 截图，',
      '生成 design.json 和 design.png 保存到指定工程目录，返回文件路径。',
      '',
      '【触发场景】',
      '当用户说以下任何一种时触发：采集设计稿、获取设计稿数据、',
      '采集设计侧数据、导出设计稿、设计稿截图、传送码采集。',
      '采集结果可直接用于 ui_style_check 进行 UI 一致性检查。',
      '',
      '【用户未提供设计稿信息时的引导】',
      '当用户只说"采集设计稿/采集设计侧数据"但没给任何设计稿信息时，不要直接调用本工具，',
      '先向用户说明需要提供以下信息（二选一）：',
      '',
      '  · 传送码（如 "111"）：通过传送码服务解析为设计稿页面地址',
      '  · 设计稿 URL：设计稿的链接地址',
      '',
      '  请把上述清单发给用户，等用户给出传送码或 URL 后再调用本工具。',
      '',
      '【参数说明】',
      '- code: 传送码，如 "111"，通过传送码服务解析为设计稿页面地址',
      '- url: 设计稿 URL 地址',
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `采集过程出错: ${err.message}\n\n可能是传送码/设计稿url提供错误，使用指南见 ${config.MCP_GUIDE_URL}`,
            },
          ],
          isError: true,
        }
  ...
  mcp.tool(
    ...
    [
      ...
      '',
      '【用户未提供任何信息时的引导】',
      '当用户只说"帮我检查设计规范/规范走查"但没给任何信息时，不要直接调用本工具或 list_design_specs，',
      '先向用户说明需要提供以下信息：',
      '',
      '  ① 检查目标（必填），二选一：',
      '     · 本地 HTML 文件路径（如 /path/to/page.html）',
      '     · Web 页面 URL（如 https://example.com/page）',
      '  ② 规范名称/场景（可选，但建议提供，不提供会返回全量列表让你选）：',
      '     · 规范名或分类名，如 "Octo"、"ICT 领域组件库" 等',
      '     · 场景名，如 "Web端_深色"、"移动端" 等',
      '',
      '  请把上述清单发给用户，等用户给出检查目标后，再先调 list_design_specs 匹配规范，最后调用本工具。',
      '',
      '【调用前：必须先匹配规范】',
