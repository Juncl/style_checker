...
export function createMcpServer() {
  ...
  mcp.tool(
    ...
    [
      ...
      '',
      '【输出格式】',
      '返回 JSON，以设计侧节点为维度组织。每个节点含 designName（设计侧节点名）、devName（开发侧节点名）、',
      'textContent、issues 列表。每条 issue 含：',
      '- severity: error（需修复）/ warning（建议修复）',
      ...
      '【输出指引】',
      '拿到结果后，按以下格式展示给用户：',
      '1. 先按 error 优先、warning 其次排序',
      '2. 每个设计节点作为一个分组，列出该节点下所有差异',
      '3. 每条差异格式：「属性描述」设计稿为 expected，开发侧为 actual，需要修改',
      '4. 不要展示评分、匹配覆盖率、检查耗时等统计信息',
      '5. 不要展示节点 ID、坐标等内部技术信息',
      '6. 如果没有差异，直接说"开发侧与设计稿一致，无需修改"',
    ].join('\n'),
    {
      designJsonPath: z
        .string()
