...
| 真实业务接口（如 `/api/check/*`） | `server/src/routes/` → Express 路由 | 3000 |
...
- ❌ 禁止：为 mock 接口添加 `/api → 3000` 的 Vite proxy 规则
...
- ✅ 新增后台管理接口：在 `client/src/api/api.ts` 追加，调用 `Api`（来自 `request.ts`）
